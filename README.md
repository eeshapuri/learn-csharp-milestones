...
- [X] ✏️ Open the reference manual for any component from inside Unity. Paste the link: https://docs.unity3d.com/6000.2/Documentation/Manual/class-Transform.html
...
- [x] ✏️ Explain some differences between public and private variables: Public variables show up in the inspector tab and can be accessed by other scripts, while private variables stay hidden. So public variables are better for things you want to change or share.
...
- Classes
	- [X] ✏️ Explain the difference between reference and value type variables: **Value types store the actual data directly (like int, float, or bool). Reference types store a reference, or ‘pointer,’ to where the data lives in memory (like string, arrays, or custom class objects). So if you copy a value type, you get a separate copy. But if you copy a reference type, both variables point to the same object.**
...
- [x] Physics and Collisions
	...
	- [x] ✏️ Q: Will a [collision detection](https://docs.unity3d.com/Manual/CollidersOverview.html) occur when a Kinematic Rigidbody Collider touches a Static Collider? : **A collision won’t actually happen in the physics sense/the objects won’t bump or stop each other. But if one has a trigger collider, Unity will still notice the overlap and you can catch it with OnTriggerEnter()**
